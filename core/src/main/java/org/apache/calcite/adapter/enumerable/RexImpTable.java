...
*/
  ...
  static class PercentRankImplementor extends RankImplementor {
    @Override protected void implementNotNullAdd(WinAggContext info,
        WinAggAddContext add) {
      Expression acc = add.accumulator().get(0);

      // The generated code is based on the similar from RankImplementor#implementNotNullAdd
      BlockBuilder builder = add.nestBlock();
      add.currentBlock().add(
          Expressions.block(
              Expressions.ifThen(
                  Expressions.lessThan(
                      add.compareRows(
                          Expressions.subtract(add.currentPosition(), Expressions.constant(1)),
                              add.currentPosition()),
                      Expressions.constant(0)),
                  Expressions.statement(
                      Expressions.assign(acc,
                          Expressions.divide(
                              Expressions.multiply(
                                  Expressions.constant(1.0), computeNewRank(acc, add)),
                              Expressions.subtract(add.getPartitionRowCount(),
                                  Expressions.constant(1))))))));
      add.exitBlock();
      add.currentBlock().add(
          Expressions.ifThen(
    ...
    @Override protected Expression implementNotNullResult(
        WinAggContext info, WinAggResultContext result) {
      // in case of PERCENT_RANK Rank is 0-based
      return Expressions.subtract(super.implementNotNullResult(info, result),
          Expressions.constant(1));
    }
  }
